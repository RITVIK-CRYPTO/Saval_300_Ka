...
class QuizApp {
  ...
  bindEvents() {
    ...
    document.getElementById('btn-view-leaderboard').addEventListener('click', () => this.openLeaderboardModal());
    document.getElementById('btn-modal-leaderboard').addEventListener('click', () => {
      this.closeModal();
      this.openLeaderboardModal();
    });
    document.getElementById('leaderboard-modal-close').addEventListener('click', () => this.closeLeaderboardModal());
    document.getElementById('btn-refresh-leaderboard').addEventListener('click', () => this.refreshLeaderboard());
    document.getElementById('btn-export-csv').addEventListener('click', () => this.exportLeaderboardCSV());
  ...
  async checkURLForQuiz() {
    ...
    // Extract quiz data from URL — support both ?param and #param formats
    // Also handle URL-encoding differences from WhatsApp/Teams/Email
    ...
    if (!rawCompressed && !blobId && hash && hash.length > 2) {
      // Unescape WhatsApp/Teams mangling
      const cleanHash = hash.replace(/%3D/gi, '=').replace(/%2B/gi, '+').replace(/%2F/gi, '/');
      if (cleanHash.startsWith('data=')) {
        rawCompressed = cleanHash.substring(5);
      } else if (cleanHash.startsWith('id=')) {
        blobId = cleanHash.substring(3);
      } else if (/^[0-9a-fA-F]{15,}$/.test(cleanHash)) {
        // Looks like a hex blob ID
        blobId = cleanHash;
      } else {
        // Treat entire hash as compressed data
        rawCompressed = cleanHash;
      }
    }

    if (rawCompressed || blobId) {
      ...
      // ── Try 1: Decompress LZString data embedded in URL (PRIMARY — always works) ──
      if (rawCompressed) {
        quizPayload = this.decompressPayload(rawCompressed);
      }

      // ── Try 2: Fetch from JSONBlob (legacy links / backup only) ──
      if (!quizPayload && blobId) {
        try {
          ...
          clearTimeout(to);
          if (res.ok) quizPayload = await res.json();
        } catch (e) {
          console.warn('JSONBlob fetch failed (rate-limited or offline):', e.message);
        }
      }

      // ── Try 3: If blobId may have been a compressed string (old format) ──
      if (!quizPayload && blobId) {
    ...
    // No quiz data in URL — show creator Home screen
    this.showView('view-home');
  }
  ...
  async generateShareLink() {
    ...
    // Create Cloud Leaderboard Blob (leaderboard-only, non-blocking)
    let leaderboardId = null;
    try {
      const ctrl = new AbortController();
      const to = setTimeout(() => ctrl.abort(), 5000);
      const lbRes = await fetch('https://jsonblob.com/api/jsonBlob', {
        method: 'POST',
      ...
      clearTimeout(to);
      if (lbRes.ok) {
        leaderboardId = lbRes.headers.get('x-jsonblob-id');
        if (!leaderboardId) {
          const loc = lbRes.headers.get('Location') || lbRes.headers.get('location');
          if (loc) leaderboardId = loc.split('/').pop();
        }
      }
    } catch (e) {
      console.warn('Cloud leaderboard creation skipped (offline/rate-limited):', e.message);
    }
    ...
    // ── PRIMARY: Embed quiz directly in URL via LZString compression ──
    // This ensures ALL 25 participants can open the link simultaneously
    // because the quiz data travels in the URL itself — no third-party API needed.
    let compressed = LZString.compressToEncodedURIComponent(JSON.stringify(quizPayload));

    // If images make URL too large (>60k chars), strip images and retry
    if (compressed.length > 60000) {
      const noImgPayload = {
        ...quizPayload,
        questions: quizPayload.questions.map(q => { const c = {...q}; delete c.image; return c; })
      };
      compressed = LZString.compressToEncodedURIComponent(JSON.stringify(noImgPayload));
      this.showToast('⚠️ Images removed to fit in sharable link. Use text questions only for sharing.');
    }
  ...
  showResults() {
    ...
    // Sync to Leaderboard
    this.saveScoreToLeaderboard(this.participantName, earnedMarks, totalMarks, correctCount);
  ...
  // ── Winner Leaderboard Methods ──
  async saveScoreToLeaderboard(name, score, totalMarks, correctCount) {
    const entry = {
      name: name,
      score: score,
      totalMarks: totalMarks,
    ...
    const quizId = (this.quizData && this.quizData.id) || 'local';
    const localKey = `cw_lb_${quizId}`;
    ...
    const existingIdx = localScores.findIndex(s => s.name.toLowerCase() === name.toLowerCase());
    if (existingIdx >= 0) {
      if (score > localScores[existingIdx].score) localScores[existingIdx] = entry;
    } else {
      localScores.push(entry);
    }
    localScores.sort((a, b) => b.score - a.score || a.timestamp - b.timestamp);
    try {
      localStorage.setItem(localKey, JSON.stringify(localScores));
    } catch (e) {}

    // 2. Cloud Storage
    const leaderboardId = this.quizData && this.quizData.leaderboardId;
    if (leaderboardId) {
      try {
        let cloudScores = [];
        const res = await fetch(`https://jsonblob.com/api/jsonBlob/${leaderboardId}`);
        if (res.ok) {
          cloudScores = await res.json();
          if (!Array.isArray(cloudScores)) cloudScores = [];
        }
        const cIdx = cloudScores.findIndex(s => s.name.toLowerCase() === name.toLowerCase());
        if (cIdx >= 0) {
          if (score > cloudScores[cIdx].score) cloudScores[cIdx] = entry;
        } else {
          cloudScores.push(entry);
        }
        cloudScores.sort((a, b) => b.score - a.score || a.timestamp - b.timestamp);

        await fetch(`https://jsonblob.com/api/jsonBlob/${leaderboardId}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(cloudScores)
        });
      } catch (e) {
        console.warn('Could not sync score to cloud leaderboard:', e);
      }
    }
  }
  ...
  async openLeaderboardModal() {
    ...
    titleEl.textContent = (this.quizData && this.quizData.title) || 'Live Winner Leaderboard';
  ...
  renderLeaderboardUI(scores) {
    ...
    if (scores.length > 0) {
      ...
      document.getElementById('podium-1-name').textContent = scores[0].name;
      document.getElementById('podium-1-score').textContent = `${scores[0].score}/${scores[0].totalMarks}`;
    } else {
    ...
    if (scores.length > 1) {
      ...
      document.getElementById('podium-2-name').textContent = scores[1].name;
      document.getElementById('podium-2-score').textContent = `${scores[1].score}/${scores[1].totalMarks}`;
    } else {
    ...
    if (scores.length > 2) {
      ...
      document.getElementById('podium-3-name').textContent = scores[2].name;
      document.getElementById('podium-3-score').textContent = `${scores[2].score}/${scores[2].totalMarks}`;
    } else {
    ...
    if (scores.length === 0) {
      listEl.innerHTML = '<div class="leaderboard-empty">No scores recorded yet. Be the first champion!</div>';
      return;
    }
    ...
    scores.forEach((s, idx) => {
      ...
      html += `
        <div class="leaderboard-row">
          ...
          <div class="leaderboard-val">${s.score} / ${s.totalMarks} (${s.percentage || Math.round((s.score/s.totalMarks)*100)}%)</div>
        </div>
      `;
    });
  ...
  exportLeaderboardCSV() {
    ...
    const doExport = (scores) => {
      if (!scores || scores.length === 0) {
        this.showToast('No scores recorded yet to export');
        return;
      }

      scores.sort((a, b) => b.score - a.score || a.timestamp - b.timestamp);

      let csv = 'Rank,Participant Name,Score,Total Marks,Percentage,Date\n';
      scores.forEach((s, idx) => {
        const dateStr = new Date(s.timestamp || Date.now()).toLocaleString();
        csv += `${idx + 1},"${s.name.replace(/"/g, '""')}",${s.score},${s.totalMarks},${s.percentage}%,${dateStr}\n`;
      });
      ...
      const quizTitle = (this.quizData && this.quizData.title) ? this.quizData.title.replace(/[^a-zA-Z0-9]/g, '_') : 'CW_Quiz';
      link.setAttribute('download', `${quizTitle}_Winner_Scorecard.csv`);
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      this.showToast('📥 Winner Scorecard CSV Downloaded!');
    };

    // Try cloud first then local
    if (leaderboardId) {
      fetch(`https://jsonblob.com/api/jsonBlob/${leaderboardId}`)
        .then(res => res.json())
        .then(cloudScores => {
          if (Array.isArray(cloudScores) && cloudScores.length > 0) doExport(cloudScores);
          else fallbackLocal();
        })
        .catch(() => fallbackLocal());
    } else {
...
class QuizApp {
  ...
  lockAnswer() {
    ...
    if (q.type === 'objective') {
      ...
      // Lock all options — no colour reveal (answers shown only at end)
      document.querySelectorAll('.kbc-option').forEach(btn => {
        btn.classList.add('disabled');
        btn.classList.remove('correct', 'wrong');
      });

      // 🔊 Neutral lock sound — does NOT reveal correct/wrong
      // (correct/wrong sounds are saved for the final results reveal)
      this.sound.playLock();

      // Move straight to next button — no delay, no colour reveal
      this.afterReveal(isCorrect);
    ...
    this.timerInterval = setInterval(() => {
      ...
      if (this.timeLeft <= 0) {
        ...
        if (!this.isAnswerLocked) {
          ...
          if (q.type === 'objective') {
            // Lock all options — no colour reveal on timeout (answers shown only at end)
            document.querySelectorAll('.kbc-option').forEach(btn => {
              btn.classList.add('disabled');
              btn.classList.remove('correct', 'wrong');
            });
